...
# PASTE YOUR NEW DISCORD BOT TOKEN BETWEEN THE QUOTES.
# DO NOT SEND THE TOKEN TO ANYONE OR POST IT ON GITHUB.
...
def get_settings(guild_id):
    CUR.execute(
        "SELECT antinuke,welcome,goodbye,autorole_id "
...
def ensure_economy(guild_id, user_id):
    CUR.execute(
        ...
        "(guild_id,user_id,wallet,bank,daily_at) VALUES(?,?,?,?,?)",
...
async def anti_nuke_check(guild, deleted_type):
    ...
    try:
        async for entry in guild.audit_logs(limit=5):
            if deleted_type == "channel":
                valid_actions = {discord.AuditLogAction.channel_delete}
            else:
                valid_actions = {discord.AuditLogAction.role_delete}
            ...
            try:
                await guild.ban(
                    actor,
                    reason=f"MIMI Anti-Nuke: {deleted_type} deletion"
                )
...
async def on_member_join(member):
    _, welcome, _, autorole_id = get_settings(member.guild.id)
...
async def on_guild_channel_delete(channel):
    await anti_nuke_check(channel.guild, "channel")


@bot.event
async def on_guild_role_delete(role):
    await anti_nuke_check(role.guild, "role")
...
async def help_command(ctx):
    e = make_embed(
        ...
        f"Prefix: `{PREFIX}`\n\n"
        "💗 **Core & Utility**\n"
        "`*help` `*ping` `*botinfo` `*uptime` `*serverinfo` "
        "`*userinfo` `*avatar` `*roleinfo` `*channelinfo` `*id` `*invite`\n\n"
        "🛡️ **Security**\n"
        "`*antinuke` `*antinuke on` `*antinuke off` "
        "`*lockdown` `*unlockdown` `*whitelist` `*unwhitelist` `*security`\n\n"
        "🔨 **Moderation**\n"
        "`*ban` `*unban` `*kick` `*mute` `*unmute` `*warn` "
        "`*warnings` `*clear` `*slowmode` `*nick`\n\n"
        "🌸 **Community**\n"
        ...
        "🔒 Payment command is owner-only and hidden."
    )
...
async def uptime(ctx):
    await ctx.send(
        embed=make_embed("⏱️ Uptime", format_uptime())
    )
...
async def channelinfo(
    ...
    await ctx.send(
        embed=make_embed(
            ...
            f"**Type:** `{channel.type}`\n"
            f"**Created:** "
            f"<t:{int(channel.created_at.timestamp())}:D>"
        )
    )
...
async def antinuke_on(ctx):
    ...
    await ctx.send(
        embed=make_embed(
            "🛡️ Anti-Nuke ON",
            "Basic channel/role deletion protection is enabled."
        )
    )
...
async def antinuke_off(ctx):
    ...
    await ctx.send(
        embed=make_embed(
            "🛡️ Anti-Nuke OFF",
            "Anti-Nuke protection is disabled."
        )
    )
...
async def unwhitelist(ctx, member: discord.Member):
    ...
    await ctx.send(
        embed=make_embed(
            "❌ Removed",
            f"{member.mention} was removed from whitelist."
        )
    )
...
async def security(ctx):
    antinuke_state, welcome, goodbye, autorole_id = get_settings(
        ctx.guild.id
    )
    ...
    await ctx.send(
        embed=make_embed(
            "🛡️ Security Status",
            f"**Anti-Nuke:** "
            f"{'🟢 ON' if antinuke_state else '🔴 OFF'}\n"
            f"**Welcome:** "
            f"{'🟢 ON' if welcome else '🔴 OFF'}\n"
            f"**Goodbye:** "
            f"{'🟢 ON' if goodbye else '🔴 OFF'}\n"
            f"**Auto Role:** {autorole}"
        )
    )
...
async def mute(
    ...
    *,
    ...
    # FIXED: timedelta comes from datetime, not discord.
...
async def warn(
    ...
    *,
    ...
    CUR.execute(
        "INSERT INTO warnings(guild_id,user_id,reason) "
        "VALUES(?,?,?)",
...
async def welcome(ctx, mode: str = None):
    if mode is None:
        ...
        return await ctx.send(
            f"Welcome system: **{'ON' if state else 'OFF'}**. "
            f"Use `{PREFIX}welcome on/off`."
        )
    ...
    await ctx.send(
        embed=make_embed(
            "🌸 Welcome",
            f"Welcome system is now **{mode.upper()}**."
        )
    )
...
async def goodbye(ctx, mode: str = None):
    if mode is None:
        ...
        return await ctx.send(
            f"Goodbye system: **{'ON' if state else 'OFF'}**. "
            f"Use `{PREFIX}goodbye on/off`."
        )
    ...
    await ctx.send(
        embed=make_embed(
            "👋 Goodbye",
            f"Goodbye system is now **{mode.upper()}**."
        )
    )
...
async def autorole(ctx, role: discord.Role = None):
    if role is None:
        ...
        current = ctx.guild.get_role(rid) if rid else None

        return await ctx.send(
            f"Auto role: "
            f"**{current.mention if current else 'OFF'}**"
        )
...
async def deposit(ctx, amount: int):
    ...
    if amount <= 0 or amount > wallet:
        return await ctx.send(
            "❌ Invalid amount or insufficient wallet balance."
        )
...
async def leaderboard(ctx):
    rows = CUR.execute(
        "SELECT user_id,wallet,bank FROM economy "
        "WHERE guild_id=? "
...
async def paycoins(
    ...
    await ctx.send(
        embed=make_embed(
            "💸 Payment Sent",
            f"{ctx.author.mention} sent **₹{amount}** "
            f"to {member.mention}."
        )
    )
...
async def choose(ctx, *, options: str):
    ...
    if len(items) < 2:
        return await ctx.send(
            "❌ Give at least 2 choices separated by commas."
        )

    await ctx.send(
        embed=make_embed(
            "🤔 MIMI Chooses",
            f"**Winner:** {random.choice(items)}"
        )
    )
...
async def giveaway(
    ...
    *,
    ...
    CUR.execute(
        ...
        "(message_id,channel_id,prize,host_id) "
        "VALUES(?,?,?,?)",
        (
            msg.id,
            ctx.channel.id,
...
async def reroll(ctx, message_id: int):
    ...
    if not row:
        return await ctx.send(
            "❌ Giveaway not found in this channel."
        )
    ...
    try:
        ...
        await ctx.send(
            "❌ Couldn't fetch that giveaway message."
        )
...
async def servericon(ctx):
    ...
    e = make_embed(
        "🖼️ Server Icon",
        f"[Open Icon]({ctx.guild.icon.url})"
    )
...
async def channels(ctx):
    text_channels = "\n".join(
        f"💬 {c.mention}"
        for c in ctx.guild.text_channels[:50]
    )

    await ctx.send(
        embed=make_embed(
            "📺 Text Channels",
            text_channels or "No text channels."
        )
    )
...
async def suggest(ctx, *, suggestion: str):
    await ctx.send(
        embed=make_embed(
            "💡 Suggestion",
            f"Thanks {ctx.author.mention}!\n"
            f"Your suggestion was received:\n"
            f"**{suggestion}**"
        )
    )
...
async def serverbanner(ctx):
    ...
    e = make_embed(
        "🖼️ Server Banner",
        f"[Open Banner]({ctx.guild.banner.url})"
    )
...
async def pay(
    ...
    *,
    ...
    if amount is None:
        return await ctx.send(
            f"❌ Usage: `{PREFIX}pay <amount> <reason>`"
        )
    ...
    # FIXED: URL-encode UPI parameters safely.
    ...
    e.set_image(
        url="attachment://mimi_payment_qr.png"
    )
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        return await ctx.send(
            f"❌ Missing argument. Use "
            f"`{PREFIX}help` for usage."
        )
...
if not TOKEN or TOKEN == "PASTE_YOUR_NEW_BOT_TOKEN_HERE":
    raise RuntimeError(
        "❌ Bot token missing. Put your NEW bot token "
